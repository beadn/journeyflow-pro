...
export function TreeView({ journey, onBlockEdit }: TreeViewProps) {
  ...
  const { nodes: initialNodes, edges: initialEdges } = useMemo(() => {
    ...
    let currentY = 0;
    // Representa el último nodo del "main flow": puede ser un trigger o el último bloque del periodo anterior
    let prevTimelineNodeId: string | null = null;
    ...
    sortedPeriods.forEach((period, periodIndex) => {
      ...
      // 1) Trigger de tiempo (esperar hasta que se cumpla la condición de tiempo)
      nodes.push({
        id: triggerId,
        type: "timeTrigger",
      ...
      // 2) Conectar el "main flow":
      // Si venimos de otro periodo, conectamos desde el último nodo del periodo anterior
      // (último bloque si había bloques, o el trigger si no había) hasta el trigger actual.
      if (prevTimelineNodeId) {
        edges.push({
          id: `timeline-${prevTimelineNodeId}-${triggerId}`,
          source: prevTimelineNodeId,
          target: triggerId,
          type: "smoothstep",
          style: grayEdgeStyle,
          animated: false,
      ...
      // 3) Bloques bajo este trigger, centrados horizontalmente
      if (periodBlocks.length > 0) {
        ...
        periodBlocks.forEach((block, blockIndex) => {
          ...
          // Conectamos el trigger a cada bloque con línea cian (fan-out)
          edges.push({
        ...
        // El nodo timeline "de salida" de este periodo es el ÚLTIMO bloque
        prevTimelineNodeId = periodBlocks[periodBlocks.length - 1].id;
      } else {
        // Si no hay bloques, el "main flow" sigue desde el trigger
        prevTimelineNodeId = triggerId;
      }
    });

    // 4) Dependencias entre bloques (cross-period / intra-period)
      ...
      <div className="absolute bottom-6 left-6 bg-white border border-gray-200 rounded-lg shadow-md p-3 z-10">
        ...
        <div className="space-y-1.5">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-gray-500">Trigger de tiempo → bloques del periodo</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-6 h-0.5 bg-gray-300 rounded" />
            <span className="text-xs text-gray-500">Flujo principal (último nodo del periodo → siguiente trigger)</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-6 h-0.5 bg-orange-500 rounded" />
            <span className="text-xs text-gray-500">Dependencia entre bloques</span>
          </div>
        </div>
      </div>
